...
function GenericProfileInfo({
  ...
  editMode,
  setCurrentUser,
}: ProfileProps<User | Club> & AuthContextI) {
  // Internal
  ...
  const saveInterests = (list: string[]) => {
    ...
    const updatedProfile = userService.updateAttribute(profile, { interests })
  ...
  const save = () => {
    const updatedProfile = userService.updateAttribute(profile, { desc: desc })
    setCurrentUser(updatedProfile)
  }
